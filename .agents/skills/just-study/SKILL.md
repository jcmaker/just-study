...
1. Probe <http://127.0.0.1:5878/api/health> with a plain HTTP request first; it answers even when the MCP client is disconnected. If nothing answers, start the server yourself: run `npm run dev` as a background process from the package root that holds this skill — three levels above this skill directory — then poll the probe until it answers, for up to 60 seconds. Start the server only from this skill; never add a session hook or any other always-on trigger, because the plugin is installed for every session on the machine and most of them are not studying. Stop and say so if it never answers, or if the port is already held by something that is not this app. Once the probe answers, call `health`. If the probe answers but the MCP tools are missing, the MCP client did not connect when this session started: tell the user to restart the session once, and stop — starting the server does not attach it to a running session. If `health` reports `state: "uninitialized"`, explain that the first approved `create_course` write will initialize this computer's empty local store, then continue the new-course flow. If it reports `ok: false`, surface its Korean `message` to the user — it describes a schema mismatch, storage inconsistency, or corrupt, orphaned, or temporary entries — and stop.
