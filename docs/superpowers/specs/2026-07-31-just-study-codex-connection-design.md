...
2026-07-31 브레인스토밍에서 승인됨. 구현 전 사용자 문서 검토 대기.
...
- OAuth, MCP 토큰, HTTPS와 원격 접속은 self-host 보안 단계에서 함께 구현한다.
...
외부 접속 단계에서는 같은 endpoint에 관리자 인증, HTTPS, 안전한 세션과 MCP 토큰을
함께 추가한다. 그 전에는 `0.0.0.0` 바인딩을 문서화하거나 허용하지 않는다.
